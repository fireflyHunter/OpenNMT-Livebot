...
class VideoEncoder(nn.Module):
    ...
    def __init__(self, d_model, d_ff, n_head, dropout, n_block, visual_output=512):
        super(VideoEncoder, self).__init__()
        self.layers = nn.ModuleList([VideoBlock(d_model, d_ff, n_head, dropout) for _ in range(n_block)])
        self.fc = nn.Linear(d_model, visual_output)
        self.norm = LayerNorm(visual_output)
...
class TextBlock(nn.Module):
    ...
    def __init__(self, d_model, d_ff, n_head, dropout):
        ...
        # self.video_attn = MultiHeadedAttention(n_head, 2048)
        self.feed_forward = PositionwiseFeedForward(d_model, d_ff, dropout)
        self.sublayer = nn.ModuleList([SublayerConnection(d_model, dropout) for _ in range(3)])
        # self.sublayer = nn.ModuleList([SublayerConnection(d_model, dropout), SublayerConnection(2048, dropout), SublayerConnection(2048, dropout)])
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, m1, m2, mask):
        ...
        x_norm = self.norm(x)
        mid, attn = self.text_attn(x_norm, m2, m2, r_attn=True)
        out = self.drop(mid) + x

        return self.sublayer[3](out, self.feed_forward), attn
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, r_attn=False):
        if mask is not None:
