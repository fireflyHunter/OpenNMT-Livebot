    ...
    def process(self, batch, device=None):
        ...
        for i, example in enumerate(batch):
            for src, tgt in example:
                ...
                # register ['N_in_batch', 'tgt_id+1', 'src_id']
...
def add_img_fields(field):
    field["image"] = image_fields()
    return field
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                if self.yield_raw_example:
                    ...
                    for data in minibatch:
                        vid = data.tgt[0][-1][4:].split(":")[0]
                        time = data.tgt[0][-1][4:].split(":")[1]
                        visual_piece = self.load_imgs(int(vid), int(time))
                        visual.append(visual_piece)
                        data.tgt[0]=data.tgt[0][:-1]
                    visual = torch.stack(visual)
                    batch = torchtext.data.Batch(minibatch,self.dataset,self.device)
                    batch.src = batch.src + (visual,)
                    yield batch
            if not self.repeat:
                return
...
class DatasetLazyIter(object):
    ...
    def __init__(self, dataset_paths, fields, batch_size, batch_size_fn,
                 batch_size_multiple, device, is_train, pool_factor, visual=None, sort_within_batch=True,
...
def build_dataset_iter(corpus_type, fields, opt, sort_within_batch=True, visual=None, is_train=True, multi=False):
    """
