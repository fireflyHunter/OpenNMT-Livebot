...
class LossComputeBase(nn.Module):
    ...
    def _compute_loss(self, batch, output, target, **kwargs):
        ...
        """
        return NotImplementedError
    def _compute_loss_cross_entropy(self, output, target, **kwargs):
        """
        Compute the loss. Subclass must define this method.
    ...
    def __call__(self,
        ...
        if test:
            loss = self._compute_loss_cross_entropy(**shard_state)
            return loss
        if shard_size == 0:
...
class NMTLossCompute(LossComputeBase):
    ...
    def _compute_loss_cross_entropy(self, output, target):
        ...
        # loss = nn.NLLLoss(ignore_index=1, reduction='none')(scores, gtruth)
        # loss = nn.CrossEntropyLoss(ignore_index=1, reduction='none')(scores, gtruth)
